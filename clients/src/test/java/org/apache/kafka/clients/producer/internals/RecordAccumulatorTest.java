...
public class RecordAccumulatorTest {
    ...
    public void testFull() throws Exception {
        ...
        for (int i = 0; i < appends; i++) {
            ...
            accum.append(tp1, 0L, key, value, -1L, null, maxBlockTimeMs);
    ...
    public void testAppendLarge() throws Exception {
        ...
        accum.append(tp1, 0L, key, new byte[2 * batchSize], -1L, null, maxBlockTimeMs);
        assertEquals("Our partition's leader should be ready", Collections.singleton(node1), accum.ready(cluster, time.milliseconds()).readyNodes);
    }
    ...
    public void testPartialDrain() throws Exception {
        ...
        for (TopicPartition tp : partitions) {
            for (int i = 0; i < appends; i++)
                accum.append(tp, 0L, key, value, -1L, null, maxBlockTimeMs);
        }
            ...
            threads.add(new Thread() {
                public void run() {
                    for (int i = 0; i < msgs; i++) {
                        try {
                            accum.append(new TopicPartition(topic, i % numParts), 0L, key, value, -1L, null, maxBlockTimeMs);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
    ...
    public void testNextReadyCheckDelay() throws Exception {
        ...
        for (int i = 0; i < appends; i++)
            accum.append(tp3, 0L, key, value, -1L, null, maxBlockTimeMs);
        ...
        for (int i = 0; i < appends + 1; i++)
            accum.append(tp2, 0L, key, value, -1L, null, maxBlockTimeMs);
        result = accum.ready(cluster, time.milliseconds());
    ...
    public void testFlush() throws Exception {
        ...
        for (int i = 0; i < 100; i++)
            accum.append(new TopicPartition(topic, i % 3), 0L, key, value, -1L, null, maxBlockTimeMs);
        RecordAccumulator.ReadyCheckResult result = accum.ready(cluster, time.milliseconds());
        assertEquals("No nodes should be ready.", 0, result.readyNodes.size());
    ...
    public void testAwaitFlushComplete() throws Exception {
        ...
        accum.append(new TopicPartition(topic, 0), 0L, key, value, -1L, null, maxBlockTimeMs);
    ...
    public void testAbortIncompleteBatches() throws Exception {
        ...
        for (int i = 0; i < 100; i++)
            accum.append(new TopicPartition(topic, i % 3), 0L, key, value, -1L, new TestCallback(), maxBlockTimeMs);
        RecordAccumulator.ReadyCheckResult result = accum.ready(cluster, time.milliseconds());
        assertEquals("No nodes should be ready.", 0, result.readyNodes.size());
    ...
    public void testExpiredBatches() throws InterruptedException {
        ...
        accum.append(tp1, 0L, key, value, -1L, null, 0);
